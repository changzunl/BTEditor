...
void Chunk::Render(int pass) const
{
	...
	{
		size_t meshIndexCount = (m_opaqueMeshCount >> 2) * 6;
	...
	{
		size_t meshIndexCount = (m_fluidMeshCount >> 2) * 6;
...
void Chunk::RebuildOpaqueMesh()
{
	...
	m_opaqueMeshCount = m_opaqueMesh.Count();
    m_opaqueMesh.Reset();
    m_opaqueMesh.Clear();
}
...
void Chunk::RebuildTranslucentMesh()
{
    ...
    m_fluidMeshCount = m_fluidMesh.Count();
	m_fluidMesh.Reset();
	m_fluidMesh.Clear();
}
