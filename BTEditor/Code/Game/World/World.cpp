...
void SpawnInfo::InitializeDefinitions(const char* path)
{
    XmlDocument document;
    document.LoadFile(path);

    const XmlElement* pElement = document.RootElement()->FirstChildElement("MapDefinition");
    while (pElement)
    {
		std::string name = ParseXmlAttribute(*pElement, "name", "");

		const XmlElement* pInfoElement = pElement->FirstChildElement("SpawnInfo");

		auto& list = s_definitions[name];

		while (pInfoElement)
        {
			list.emplace_back();
			auto& back = list.back();
			back.LoadFromXmlElement(*pInfoElement);

            pInfoElement = pInfoElement->NextSiblingElement("SpawnInfo");
		}
        pElement = pElement->NextSiblingElement("MapDefinition");
    }
}

const std::vector<SpawnInfo>& SpawnInfo::GetByName(const std::string& name)
{
	static std::vector<SpawnInfo> empty = {};

	auto ite = s_definitions.find(name);
	if (ite != s_definitions.end())
		return ite->second;
	return empty;
}

std::map<std::string, std::vector<SpawnInfo>> SpawnInfo::s_definitions;
...
World::World()
	: m_navMesh(new NavMesh2D(this, IntVec3(0, 0, 5), IntVec2(127, 127)))
{
}
...
void World::Initialize()
{
	...
	{
		m_chunkManager = new ChunkProvider(this, "Map/World", new PlainWorldGenerator());
	}
	...
	info.m_position = Vec3(0, 0, 5);
	m_player[0]->Possess(AddEntity(info));


	auto& entinfos = SpawnInfo::GetByName("World");

	for (auto& entinfo : entinfos)
	{
		AddEntity(entinfo);
	}
...
// 
	...
	if (watch.IsStopped() || watch.HasDurationElapsed())
	{
		watch.Start(2);
	...
	if (g_theInput->WasKeyJustPressed(KEYCODE_N))
	{
		...
		m_navMesh->BuildNav();
		DebugAddMessage(Stringf("Nav mesh built (%.4fs)", GetCurrentTimeSeconds() - time), 2.0f, Rgba8::WHITE, Rgba8::WHITE);
	}
...
void World::Render() const
{
	...
	debugCamera = g_theGame->GetCurrentScene()->GetWorldCamera();
...
// 
// 	g_theRenderer->DrawPostprocessRect();
// 
// 	g_theRenderer->BindShader(nullptr);
...
void World::UpdateEnvVariables() const
{
	...
	double time = GetClock()->GetTotalTime();


	// World Time
    envConsts.WORLD_TIME = (float)time;
    time = 0.05; // force noon
	double dummyIntegerPart;
	float timeOfDay = (float)modf(time + 0.25f, &dummyIntegerPart);
	...
	// Environment Values
	envConsts.LIGHTNING_VALUE = Compute1dPerlinNoise((float)time * 200.0f, 1.f, 9, 0.5f, 2.0f, true, 0);
	envConsts.LIGHTNING_VALUE = RangeMapClamped(envConsts.LIGHTNING_VALUE, 0.6f, 0.9f);
	envConsts.FLICKER_VALUE = Compute1dPerlinNoise((float)time * 200.0f, 1.f, 9, 0.5f, 2.0f, true, 0);
...
void World::RenderWorld() const
{
	...
	{
		g_theRenderer->ClearScreen(m_envConsts.SKY_COLOR.GetAsRgba8());
		...
		g_theRenderer->SetTintColor(Rgba8(255, 255, 255, 127));
        for (auto& chunkEntry : m_chunkManager->GetLoadedChunks())
...
void World::AISenseMakeNoise(const Vec3& location ,float loudness)
{
	;
}
