...
#include "Game/Scene/SceneTestJobs.hpp"
...
void SceneAttract::RenderUILogoText() const
{
	...
	// Logo
	Vec2 textPosition = g_theRenderer->GetViewport().GetPointAtUV(Vec2(0.5f, 0.75f));
	float fontSize = Clamp(GetLifeTime() * 300.0f, 0.0f, 100.0f);
	AddVertsForText(verts, "BEHAVIOR TREE DEMO", 1.0f, 0.5f);
	...
	AddVertsForText(verts, "[SPACE] TO START!", 2.0f, 0.5f);
	...
	AddVertsForText(verts, "[E] FOR BT EDITOR!", 2.0f, 0.5f);
	RenderFontVertices((int)verts.size(), &verts[0], textPosition, Rgba8(255, 255, 0, 255), fontSize, true);
	verts.clear();
}
...
// 	}
	...
	{
		SceneEditor* editor = new SceneEditor(m_game);
		m_game->LoadScene(editor);
		m_game->m_clickSound->PlaySound();
		return;
	}
