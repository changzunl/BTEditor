...
#include "Game/World/BlockIterator.hpp"
...
public:
    ...
    static const std::vector<SpawnInfo>& GetByName(const std::string& name);
...
private:
    static std::map<std::string, std::vector<SpawnInfo>> s_definitions;
};
...
public:
    ...
    void                          AISenseMakeNoise(const Vec3& location ,float loudness);
