...
#include "Game/Misc/RenderUtils.hpp"
...
{
    m_editor = new SceneEditor(m_game);
	m_editor->Initialize();
	m_editor->m_editorUI->SetInputEnabled(false);

	g_theWindow->SetFocus();
...
void ScenePlaying::Update()
{
	...
	if (m_editor)
	{
		m_editor->m_editorUI->SetDebugAI(AI::FindActive(0));
...
void ScenePlaying::RenderHUD() const
{
	...
	AABB2 screenBox = g_theRenderer->GetViewport();
	...
	Actor* player = m_map->m_player[0]->GetActor();
	Health* health = player->GetComponent<Health>();
	float hurtEffect = health->GetDamageEffect();
	if (hurtEffect > 0)
    {
        AddVertsForAABB2D(verts, screenBox, RgbaF(1, 0, 0, hurtEffect * 0.333f).GetAsRgba8());
        g_theRenderer->DrawVertexArray((int)verts.size(), verts.data());
        verts.clear();
	}
}
...
void ScenePlaying::HandleInput()
{
	...
	g_theWindow->SetMouseMode(true, false, true);
}
