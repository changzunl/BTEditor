...
bool ActorDefinition::LoadFromXmlElement(const XmlElement& element)
{
	...
	{
		m_aiEnabled   = true;
		m_aiBehavior  = ParseXmlAttribute(*ai, "aiBehavior", m_aiBehavior);
...
//         m_meleeActor = ParseXmlAttribute(*ai, "meleeActor", m_meleeActor);
	...
	if (appearance)
    {
		...
		const XmlElement* skel = appearance->FirstChildElement("SkeletalMesh");

		if (skel)
        {
            m_mesh = ParseXmlAttribute(*skel, "res", m_mesh);
            m_meshTexture = ParseXmlAttribute(*skel, "texture", m_mesh);
		}
