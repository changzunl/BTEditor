...
void App::Startup()
{
    ...
    windowConfig.m_inputSystem = g_theInput;
    windowConfig.m_windowTitle = "BT Editor";
    g_editorWindow = new Window(windowConfig);
...
void App::Shutdown()
{
    ...
    ShutdownEditorWindow();

    delete g_editorWindow;
...
void App::StartEditorWindow()
{
    if (g_editorWindow->GetHwnd() == nullptr)
    {
        g_editorWindow->Startup();
        g_theRenderer->AddScreen(g_editorWindow);
    }
}

void App::ShutdownEditorWindow()
{
    if (g_editorWindow->GetHwnd() != nullptr)
    {
        g_theRenderer->RemoveScreen(g_editorWindow);
        g_editorWindow->Shutdown();
    }
}
