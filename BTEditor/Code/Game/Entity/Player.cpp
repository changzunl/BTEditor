...
{
	PLAYER_ENABLE_EDIT = g_gameConfigBlackboard.GetValue("enableEdit", false);
...
void Player::OnRenderActor()
{
	VertexList verts;

	AABB3 aabb;
    aabb.m_mins.x = (float)Min(m_areaStart.x, m_areaEnd.x);
    aabb.m_mins.y = (float)Min(m_areaStart.y, m_areaEnd.y);
    aabb.m_mins.z = (float)Min(m_areaStart.z, m_areaEnd.z);
    aabb.m_maxs.x = (float)Max(m_areaStart.x, m_areaEnd.x);
    aabb.m_maxs.y = (float)Max(m_areaStart.y, m_areaEnd.y);
    aabb.m_maxs.z = (float)Max(m_areaStart.z, m_areaEnd.z);

	aabb.m_maxs += Vec3(1, 1, 1);
...
void Player::HandleMoveFromKeyboard(float deltaSeconds)
{
	...
	else
    {
		...
		if (acceleration.GetLength() > 0)
			acceleration.NormalizeAndGetPreviousLength();

        if (g_theInput->WasKeyJustPressed(KEYCODE_LSHIFT))
        {
			DebugAddMessage("accel", 2.0f, Rgba8::WHITE, Rgba8::WHITE);

			if (acceleration.GetLength() > 0)
				acceleration *= owner->m_definition->m_runSpeed / owner->m_definition->m_walkSpeed * 10;
			else
				acceleration += iBasis * owner->m_definition->m_runSpeed / owner->m_definition->m_walkSpeed * 10;
        }
		...
		owner->m_physics->AddForce((acceleration * owner->m_definition->m_walkSpeed) * owner->m_definition->m_drag);
	}
	...
	if (actor)
	{
		...
		if (g_theInput->WasKeyJustPressed(KEYCODE_LEFT_MOUSE))
		{
			...
			if (result.m_hitBlock)
            {
                ...
                {
                    const char* text = "Begin bulk select pos 1 %d,%d,%d";
                    WorldCoords w = result.m_blockIte.GetWorldCoords();
                    DebugAddMessage(Stringf(text, w.x, w.y, w.z), 5.0f);
                    m_areaStart = w;
                }
                else if (g_theInput->IsKeyDown(KEYCODE_LSHIFT))
                {

                }
                else
                {
					if (PLAYER_ENABLE_EDIT)
        ...
        if (g_theInput->WasKeyJustPressed(KEYCODE_MIDDLE_MOUSE))
        {
            if (g_theInput->IsKeyDown(KEYCODE_LCTRL))
            {
                const char* text = "Begin bulk set";
                DebugAddMessage(Stringf(text), 5.0f);

                int minX = Min(m_areaStart.x, m_areaEnd.x);
                int minY = Min(m_areaStart.y, m_areaEnd.y);
                int minZ = Min(m_areaStart.z, m_areaEnd.z);
                int maxX = Max(m_areaStart.x, m_areaEnd.x);
                int maxY = Max(m_areaStart.y, m_areaEnd.y);
                int maxZ = Max(m_areaStart.z, m_areaEnd.z);

                for (int k = minZ; k <= maxZ; k++)
                    for (int j = minY; j <= maxY; j++)
						for (int i = minX; i <= maxX; i++)
						{
							actor->m_world->GetChunkManager()->SetBlockId(IntVec3(i, j, k), PLAYER_INVENTORY[m_inventoryIndex]);
						}
            }
            else if (g_theInput->IsKeyDown(KEYCODE_LSHIFT))
            {
            ...
            else
            {
                ...
                if (result.m_hitBlock)
                {
                    BlockId block = result.m_blockIte.GetBlock()->GetBlockId();

                    for (int i = 0; i < PLAYER_INVENTORY_SIZE; i++)
                    {
                        if (PLAYER_INVENTORY[i] == block)
                            m_inventoryIndex = i;
                    }
                    const char* text = "Picked block %d,%d,%d(Chunk %d,%d Block %d,%d,%d)";
                    WorldCoords w = result.m_blockIte.GetWorldCoords();
                    LocalCoords l = Chunk::GetLocalCoords(w);
                    ChunkCoords c = Chunk::GetChunkCoords(w);
                    DebugAddMessage(Stringf(text, w.x, w.y, w.z, c.x, c.y, l.x, l.y, l.z), 5.0f);
		...
		if (g_theInput->WasKeyJustPressed(KEYCODE_RIGHT_MOUSE))
		{
			...
			if (result.m_hitBlock)
			{
				if (g_theInput->IsKeyDown(KEYCODE_LCTRL))
                {
                    const char* text = "Begin bulk select pos 2 %d,%d,%d";
                    WorldCoords w = result.m_blockIte.GetWorldCoords();
                    DebugAddMessage(Stringf(text, w.x, w.y, w.z), 5.0f);
                    m_areaEnd = w;
				}
